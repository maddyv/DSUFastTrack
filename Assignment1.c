...
{    
    ...
    {
    	comp i1;
        i1.real=(double)-(*b2/(2.0*(*a2))); // Standard Formula (real part)
        i1.img=(double) (sqrt(-*dcr1))/(2*(*a2)); // Standard Formula (complex part)
        printf("There are two complex roots i.e. %.2f + %.2fi and %.2f - %.2fi",i1.real,i1.img,i1.real,i1.img);
    }
}
